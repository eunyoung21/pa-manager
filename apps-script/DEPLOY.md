...
## 5. 계약서 메일 발송 켜기 (2026-08-05 추가 — 이미 배포된 백엔드도 이 과정 필요)
계약서를 메일로 보내려면 **Gmail 발송 권한**이 새로 필요하다. 코드만 붙여넣고 재배포하면 권한이 없어 발송이 실패한다.

👉 **`배포도우미.html` 더블클릭** — 순서대로 복사 버튼만 누르면 되게 만들어 둔 페이지.
   코드가 바뀌면 `node apps-script/make-deploy-helper.mjs` 로 다시 굽는다(파일 내용을 그대로 박아 넣음).
...
1. **코드·매니페스트 갱신**: 편집기에서 `Code.gs`·`appsscript.json` 을 이 폴더 최신본으로 교체 후 저장
   (`appsscript.json` 에 `script.send_mail`·`gmail.send`·`gmail.settings.basic` 스코프가 들어있어야 함.
    매니페스트의 `oauthScopes` 가 우선하므로 이 파일을 안 바꾸면 코드만 최신이어도 권한 오류)
