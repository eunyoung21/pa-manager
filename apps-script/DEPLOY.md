...
## 모두싸인 전자서명 (계약서 생성 → ✍️ 모두싸인)

1. 모두싸인 고객센터에 API 사용 신청 → 승인되면 설정 → 워크스페이스 관리 → API KEY 에서 키 발급(한 번만 보이니 바로 복사).
2. Apps Script 편집기 → 프로젝트 설정 → 스크립트 속성에 추가:
   - `MODUSIGN_EMAIL` = 모두싸인 로그인 이메일
   - `MODUSIGN_KEY`   = 발급받은 API 키
   (키는 여기에만 둔다. index.html 은 공개 저장소라 절대 넣지 않는다.)
3. 배포 관리 → 새 버전으로 다시 배포.
4. 보낸 기록은 시트 `_modusign` 에 쌓이고, 서명이 끝나면 서명본 PDF가 브랜드 계약서 폴더에 표준 이름으로 들어간다.

로컬(복사본)에서 실제 발송 시험: `_local/modusign-key.json` 에 `{"email":"…","key":"…"}` 를 두고 개발 서버를 다시 켜면
실제 모두싸인으로 나간다(없으면 흉내만). 라이브 시트·드라이브는 건드리지 않는다.
