...
2. **동시편집 중 방금 친 값이 사라짐(해결됨, 2026-09-01)**: 저장이 `conflict` 로 되돌아오면 앱은 서버 최신본을 다시 받아(앱스스크립트라 2~6초) 병합한 결과로 화면을 덮어쓴다. 그런데 ①병합 기준(`mine`)을 **요청 보낼 때** 스냅샷으로 잡아 재조회 동안 친 입력이 병합 결과에서 빠졌고, ②서버본 적용 뒤 5초는 내용이 달라도 자동저장을 통째로 건너뛰어(`skipSaveRef`) 그 입력이 저장도 안 됐다 → 영상검수 광고코드가 화면·서버 양쪽에서 사라짐. 고침: `pullMerge` 는 mine·base 를 **응답 도착 후에** 읽고(함수로 전달), `skipSaveRef` 는 **내용이 같을 때만** 저장을 생략하며, 폴링도 항상 3-way 병합한다. 재발방지 테스트 `test/review-code-conflict.e2e.mjs`.
3. **낡은 탭이 서버를 덮어씀(해결됨)**: 프론트가 localStorage(`pa_mgr_v5`)에 캐시 → 옛 탭이 자동저장으로 서버를 덮어쓸 수 있음. 데이터 수정 후엔 **모든 탭/기기 새로고침** 필요. `serverLoadedRef`로 마운트 시 덮어쓰기를 막아둠.
